...
class CRNN(nn.Module):
    ...
    def forward(self, input):
        ...
        conv = self.cnn(input)
        b, c, h, w = conv.size()
        # print('from crnn.py conv.size() for b, c, h, w: ', conv.size()) # [24, 512, 1, 71]
        assert h == 1, "the height of conv must be 1"
        conv = conv.squeeze(2) # batch, 512, width  # [b, c, h, w] -> [b, c, w]
        conv = conv.permute(2, 0, 1)  # [w, b, c]
        # output = self.rnn(conv)
        output = F.log_softmax(self.rnn(conv), dim=2) #
        # print('from crnn.py output.size() : ', output.size())
